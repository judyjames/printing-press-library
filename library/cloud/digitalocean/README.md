...
The recommended path installs both the `digitalocean-pp-cli` binary and the `pp-digitalocean` agent skill in one shot:
...
```bash
npx skills add mvanhorn/printing-press-library/cli-skills/pp-digitalocean -g
```

Then invoke `/pp-digitalocean <query>` in Claude Code. The skill is the most efficient path — Claude Code drives the CLI directly without an MCP server in the middle.
...
```bash
claude mcp add digitalocean digitalocean-pp-mcp -e DIGITALOCEAN_BEARER_AUTH=<your-token>
```
