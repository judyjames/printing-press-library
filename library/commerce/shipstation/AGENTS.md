# ShipStation Printed CLI Agent Guide

This directory is a generated `shipstation-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
shipstation-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
shipstation-pp-cli which "<capability>" --json
shipstation-pp-cli <command> --help
```
...
```bash
shipstation-pp-cli <command> --agent
```
...
```bash
shipstation-pp-cli <command> --help
shipstation-pp-cli <command> --dry-run --agent
```
