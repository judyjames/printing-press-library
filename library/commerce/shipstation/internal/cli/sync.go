...
Exit codes & warnings:
  ...
  shipstation-pp-cli sync

  # Sync specific resources only
  shipstation-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  shipstation-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  shipstation-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  shipstation-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  shipstation-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("shipstation-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/shipstation-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "carriers":
		return db.UpsertCarriers(data)
	case "options":
		return db.UpsertOptions(data)
	case "carriers_packages":
		return db.UpsertCarriersPackages(data)
	case "services":
		return db.UpsertServices(data)
	case "environment":
		return db.UpsertEnvironment(data)
	case "fulfillments":
		return db.UpsertFulfillments(data)
	case "inventory":
		return db.UpsertInventory(data)
	case "inventory_locations":
		return db.UpsertInventoryLocations(data)
	case "inventory_warehouses":
		return db.UpsertInventoryWarehouses(data)
	case "cancel_refund":
		return db.UpsertCancelRefund(data)
	case "return":
		return db.UpsertReturn(data)
	case "track":
		return db.UpsertTrack(data)
	case "void":
		return db.UpsertVoid(data)
	case "pickups":
		return db.UpsertPickups(data)
	case "products":
	...
	case "receives":
		return db.UpsertReceives(data)
	case "shipping_details":
		return db.UpsertShippingDetails(data)
	case "status":
	...
	case "internal_notes":
		return db.UpsertInternalNotes(data)
	case "shipments_rates":
		return db.UpsertShipmentsRates(data)
	case "shipments_tags":
		return db.UpsertShipmentsTags(data)
	case "suppliers":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"environment":          "/v2/environment/webhooks",
		"fulfillments":         "/v2/fulfillments",
		"inventory":            "/v2/inventory",
		"inventory-locations":  "/v2/inventory_locations",
		"inventory-warehouses": "/v2/inventory_warehouses",
		"labels":               "/v2/labels",
		"manifests":            "/v2/manifests",
		"packages":             "/v2/packages",
		"pickups":              "/v2/pickups",
		"products":             "/v2/products",
		"purchase-orders":      "/v2/purchase_orders",
		"suppliers":            "/v2/suppliers",
		"tags":                 "/v2/tags",
		"totes":                "/v2/totes",
		"totes-quantities":     "/v2/totes/quantities",
		"users":                "/v2/users",
		"warehouses":           "/v2/warehouses",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "errors", ParentTable: "batches", ParentIDParam: "batch_id", PathTemplate: "/v2/batches/{batch_id}/errors"},
	}
}
