...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "shipstation-pp-cli",
		Short: `Shipstation CLI — Every ShipStation v2 endpoint plus rate-quote history, batch-error triage, cost roll-ups, and inventory velocity that n…`,
		Long: `Shipstation CLI — Every ShipStation v2 endpoint plus rate-quote history, batch-error triage, cost roll-ups, and inventory velocity that n…
...
Health check: run 'shipstation-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("shipstation-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'shipstation-pp-cli profile list')")
	...
	batchesCmd := newBatchesCmd(flags)
	batchesCmd.AddCommand(newBatchTriageCmd(flags))
	batchesCmd.AddCommand(newBatchRetryCmd(flags))
	rootCmd.AddCommand(batchesCmd)
	rootCmd.AddCommand(newCarriersCmd(flags))
	rootCmd.AddCommand(newEnvironmentCmd(flags))
	rootCmd.AddCommand(newFulfillmentsCmd(flags))
	inventoryCmd := newInventoryCmd(flags)
	inventoryCmd.AddCommand(newInventoryDriftCmd(flags))
	inventoryCmd.AddCommand(newInventoryVelocityCmd(flags))
	rootCmd.AddCommand(inventoryCmd)
	rootCmd.AddCommand(newInventoryLocationsCmd(flags))
	rootCmd.AddCommand(newInventoryWarehousesCmd(flags))
	labelsCmd := newLabelsCmd(flags)
	labelsCmd.AddCommand(newLabelsCostCmd(flags))
	rootCmd.AddCommand(labelsCmd)
	rootCmd.AddCommand(newMailingCmd(flags))
	rootCmd.AddCommand(newManifestsCmd(flags))
	...
	rootCmd.AddCommand(newTotesCmd(flags))
	rootCmd.AddCommand(newWarehousesCmd(flags))
	...
	rootCmd.AddCommand(newRateHistoryCmd(flags))
	rootCmd.AddCommand(newEodCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("shipstation-pp-cli %s\n", version)
		},
	}
}
