...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  shipstation-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  shipstation-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"batches", "carriers", "environment", "fulfillments", "inventory", "inventory-locations", "inventory-warehouses", "labels", "manifests", "packages", "pickups", "products", "purchase-orders", "suppliers", "tags", "totes", "totes-quantities", "users", "warehouses"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  shipstation-pp-cli workflow status

  # Show status as JSON
  shipstation-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("shipstation-pp-cli")
			}
