...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".shipstation-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHIPSTATION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHIPSTATION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "shipstation-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.shipstation-pp-cli/feedback.jsonl.
When ` + "`SHIPSTATION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHIPSTATION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shipstation-pp-cli feedback list
  shipstation-pp-cli feedback list --limit 5
  shipstation-pp-cli feedback list --json`,
