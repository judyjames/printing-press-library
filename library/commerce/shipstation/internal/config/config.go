...
type Config struct {
	...
	Path              string            `toml:"-"`
	ShipstationApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SHIPSTATION_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "shipstation-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SHIPSTATION_API_KEY"); v != "" {
		cfg.ShipstationApiKey = v
		cfg.AuthSource = "env:SHIPSTATION_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ShipstationApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SHIPSTATION_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ShipstationApiKey
	if token == "" {
		return ""
	}
	if c.ShipstationApiKey == "" {
		return ""
	}
	return token
}
